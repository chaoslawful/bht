...
#include "03_memory_backend.h"	// 载入单元测试的声明，请根据实际文件名自行修改
#include <boost/shared_ptr.hpp>

#include "MemoryBackend.h"

CPPUNIT_TEST_SUITE_REGISTRATION( MemoryBackendUnitTest/*此处写入单元测试类名*/);

using namespace ::std;
using namespace ::BHT;
using namespace ::boost;

void MemoryBackendUnitTest::setUp()
{
	// 记录测试开始时刻以便输出耗时信息
...
void MemoryBackendUnitTest::tearDown()
{
	struct timeval end;
	double elapsed;
...
void MemoryBackendUnitTest::test_factory()
{
	MemoryBackendFactory factory;
	CPPUNIT_ASSERT_NO_THROW(factory.getBackendInstance("hello", 1));
}

void MemoryBackendUnitTest::test_function()
{
	MemoryBackendFactory factory;
	shared_ptr<Backend> p;
	CPPUNIT_ASSERT_NO_THROW(p = factory.getBackendInstance("hello",1));
